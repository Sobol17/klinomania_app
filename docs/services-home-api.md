...
> **Важно:** разделы этого документа про предварительный расчёт (`service-quotes`) и `quote_id` устарели и не являются контрактом. Для оформления заявки используется только [order-checkout-api.md](order-checkout-api.md): один `POST /client/orders`, без площади и без `quote_id`.
